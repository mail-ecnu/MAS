...
class Levers(object):
    ...
    def train(self, nepisodes):
        ...
        for i in range(nepisodes):
            ...
            _, loss, repeat_reward, _one_hot = self.get_reward(action_prob, _baseline)
            ...
            self.comm_in = self.comm_in.detach()

            self.prev_hid.data.zero_()
            self.comm_in.data.zero_()

            if self.opts['model'] == 'lstm':
                self.prev_cell = self.prev_cell.detach()
                self.prev_cell.data.zero_()

    def get_reward(self, action_prob, baselines):
        # sample an output
        # import pdb; pdb.set_trace()
        m = torch.distributions.Categorical(action_prob)
        lever_output = m.sample()
        # import pdb; pdb.set_trace()
        ...
        printable_reward = (reward.sum(0) / self.batch_size/ self.opts['reward_mult']).item()
        ...
        loss = torch.mean(-(repeat_reward - 0.0 * baselines).squeeze() * m.log_prob(lever_output))
